...
public class LowerExpression implements BooleanExpression {
	...
	private final static Logger LOGGER = Logger.getLogger(LowerExpression.class.getName());
		...
		if (this.expression1.getType() == Expression.INT) {
			...
			if (this.expression1.getType() == Expression.REAL) {
				...
				if ((londIDExpression1.getSymbol() != null) && (londIDExpression2.getSymbol() != null)) {
					...
					int i = 0;
					BooleanExpression simplifiedExpression = new ImpliesExpression(new LongIDExpression(
							londIDExpression2.getNormalizedLongID()
									...
									+ enumAttribute2.getBooleanAttribute(
											((EnumSetExpressionSymbol) enumAttribute2.getSetExpressionSymbol())
													.getContainedValues().get(i).toString()).getID(), null),
							new FalseExpression());
					i++;
					while (i <= enumAttribute1.getNumberOfBooleanAttributes() - 1) {
						if (i == 1) {
							simplifiedExpression = new AndExpression(simplifiedExpression,
									new ImpliesExpression(new LongIDExpression(londIDExpression2.getNormalizedLongID()
											+ "."
											+ enumAttribute2.getBooleanAttribute(
													((EnumSetExpressionSymbol) enumAttribute2.getSetExpressionSymbol())
															.getContainedValues().get(i).toString()).getID(), null),
											new LongIDExpression(londIDExpression1.getNormalizedLongID()
													+ "."
													+ enumAttribute1.getBooleanAttribute(
															((EnumSetExpressionSymbol) enumAttribute1
																	.getSetExpressionSymbol()).getContainedValues()
																	.get(i - 1).toString()).getID(), null)));
						} else {
							int j = 0;
							OrExpression orExpression = new OrExpression(new LongIDExpression(
									londIDExpression1.getNormalizedLongID()
											+ "."
											+ enumAttribute1.getBooleanAttribute(
													((EnumSetExpressionSymbol) enumAttribute1.getSetExpressionSymbol())
															.getContainedValues().get(j).toString()).getID(), null),
									new LongIDExpression(londIDExpression1.getNormalizedLongID()
											+ "."
											+ enumAttribute1.getBooleanAttribute(
													((EnumSetExpressionSymbol) enumAttribute1.getSetExpressionSymbol())
															.getContainedValues().get(j + 1).toString()).getID(), null));
							j = j + 2;
							while (j < i) {
								orExpression = new OrExpression(orExpression, new LongIDExpression(
										londIDExpression1.getNormalizedLongID()
												+ "."
												+ enumAttribute1.getBooleanAttribute(
														((EnumSetExpressionSymbol) enumAttribute1
																.getSetExpressionSymbol()).getContainedValues().get(j)
																.toString()).getID(), null));
								j++;
							}
							simplifiedExpression = new AndExpression(simplifiedExpression, new ImpliesExpression(
									new LongIDExpression(londIDExpression2.getNormalizedLongID()
											+ "."
											+ enumAttribute2.getBooleanAttribute(
													((EnumSetExpressionSymbol) enumAttribute2.getSetExpressionSymbol())
															.getContainedValues().get(i).toString()).getID(), null),
					...
					if (londIDExpression1.getSymbol() != null) {
						...
						AttributeSymbol enumAttribute1 = (AttributeSymbol) londIDExpression1.getSymbol();
						int i = 0;
						while (!(enumValue.equals(((EnumSetExpressionSymbol) enumAttribute1.getSetExpressionSymbol())
								.getContainedValues().get(i).toString()))) {
							i++;
						}
						if (i == 0) {
							return new FalseExpression();
						} else {
							BooleanExpression simplifiedExpression = new NotExpression(new LongIDExpression(
									londIDExpression1.getNormalizedLongID()
											+ "."
											+ enumAttribute1.getBooleanAttribute(
													((EnumSetExpressionSymbol) enumAttribute1.getSetExpressionSymbol())
							...
							while (i <= enumAttribute1.getNumberOfBooleanAttributes() - 1) {
								simplifiedExpression = new AndExpression(simplifiedExpression, new NotExpression(
										new LongIDExpression(londIDExpression1.getNormalizedLongID()
												...
												+ enumAttribute1.getBooleanAttribute(
														((EnumSetExpressionSymbol) enumAttribute1
																.getSetExpressionSymbol()).getContainedValues().get(i)
																.toString()).getID(), null)));
								i++;
							}
							return simplifiedExpression;
						...
						while (!(enumValue.equals(((EnumSetExpressionSymbol) enumAttribute2.getSetExpressionSymbol())
								.getContainedValues().get(i).toString()))) {
							if (simplifiedExpression == null) {
								simplifiedExpression = new NotExpression(new LongIDExpression(
										londIDExpression2.getNormalizedLongID()
												+ "."
												+ enumAttribute2.getBooleanAttribute(
														((EnumSetExpressionSymbol) enumAttribute2
																.getSetExpressionSymbol()).getContainedValues().get(i)
																.toString()).getID(), null));
							} else {
								simplifiedExpression = new AndExpression(simplifiedExpression, new NotExpression(
