...
public class FeatureTree {
	...
	public FeatureSymbol findFeatureByLongId(String longFeatureName) throws Exception {
		...
		if (names.length > 0) {
			...
			if (root.getID().equals(names[0]))
				f = root;
			else
				throw new Exception("Inexistent root : " + longFeatureName);
			boolean isRoot = true;
			for (String name : names) {
				if (!isRoot) {
					f = f.getChildrenFeature(name);
					if (f == null)
						throw new Exception("Inexistent feature : " + longFeatureName);
				} else
					isRoot = false;
			}
			return f;
	...
	private FeatureSymbol findFeatureByShortId(String shortFeatureName, FeatureSymbol start) {
		...
		if (start.getID().equals(shortFeatureName))
			return start;
		if (start.getChildrenFeature(shortFeatureName) != null)
			return start.getChildrenFeature(shortFeatureName);
		else {
